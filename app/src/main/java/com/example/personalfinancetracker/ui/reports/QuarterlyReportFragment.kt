...
class QuarterlyReportFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 🚩 CHANGE 1: Observe the QUARTERLY expense data
        viewModel.totalQuarterlyExpense.observe(viewLifecycleOwner) { expense ->
            val expenseValue = expense ?: 0.0
            binding.tvTotalExpense.text = currencyFormat.format(expenseValue)
            // Use quarterly income value for net calculation
            updateNet(expenseValue, viewModel.totalQuarterlyIncome.value)
        }

        // 🚩 CHANGE 2: Observe the QUARTERLY income data
        viewModel.totalQuarterlyIncome.observe(viewLifecycleOwner) { income ->
            val incomeValue = income ?: 0.0
            binding.tvTotalIncome.text = currencyFormat.format(incomeValue)
            // Use quarterly expense value for net calculation
            updateNet(viewModel.totalQuarterlyExpense.value, incomeValue)
        }
    }
